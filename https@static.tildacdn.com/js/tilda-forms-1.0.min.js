...
break;case 'url':regExp=/^((https?|ftp):\/\/)?[a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéûşç0-9][a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéûşç0-9_\.\-]{0,253}\.[a-zA-Zёа-яЁА-Я]{2,10}\/?$/gi;if(val.length>0){str=val.split('../https@/');if(str&&str.length>1){str=str[1]}else{str=str[0]}
...
window.stripehandler.open({name:companyname,image:companylogo,description:arArgs.description,amount:parseInt(parseFloat(arArgs.amount)*multiple),currency:arArgs.currency,shippingAddress:arArgs.shipping=='1'?!0:!1,email:arArgs.email>''?arArgs.email:'',token:function(token,args){if(token&&token.id){$.ajax({type:"POST",url:'../https@forms.tildacdn.com/payment/stripe/default.htm',data:{projectid:arArgs.projectid,invoiceid:arArgs.invoiceid,token:token.id,email:token.email,currency:arArgs.currency,amount:parseInt(parseFloat(arArgs.amount)*multiple)},dataType:"json",xhrFields:{withCredentials:!1},success:function(json){window.tildaForm.clearTCart($jform);var virtPage='/tilda/'+$jform.attr('id')+'/payment/';var virtTitle='Pay order in form '+$jform.attr('id');var virtPrice=arArgs.amount;var virtProduct=arArgs.description;if(window.Tilda&&typeof Tilda.sendEventToStatistics=='function'){var currency=$('#allrecords').data('tilda-currency')||$('.t706').data('project-currency-code');if(!currency){$('#allrecords').data('tilda-currency',arArgs.currency)}
Tilda.sendEventToStatistics(virtPage,virtTitle,virtProduct,virtPrice)}
if(arArgs.successurl>''){window.setTimeout(function(){window.location.href=arArgs.successurl},300)}
if($jform.data('successmessage')>''){$jform.find('.js-successbox').html($jform.data('successmessage'))}else{$jform.find('.js-successbox').html('')}
...
$.ajax({type:"POST",url:'../https@forms.tildacdn.com/payment/banktransfer/default.htm',data:$jbankform.serialize(),dataType:"json",xhrFields:{withCredentials:!1},success:function(json){$('body').removeClass('t-body_popupshowed');$jbankform.closest('.t-banktransfer').remove();if(!json){json={error:'Unknown error. Please reload page and try again later.'}}
...
$jform.find('.js-form-spec-comments').val('');$formurl='../https@forms.tildacdn.com/procces/default.htm';var d={};d=$jform.serializeArray();if(window.tildaForm.tildapayment&&window.tildaForm.tildapayment.products){d.push({name:'tildapayment',value:JSON.stringify(window.tildaForm.tildapayment)})}
...
$jform.data('tildaformresult',formres);var virtPage=$jform.data('tilda-event-name')||'';if(!virtPage||virtPage==''){if($jform.data('formcart')=='y'&&json&&((json.next&&json.next.type&&json.next.type!='function')||!json.next)){virtPage='tilda/'+$jform.attr('id')+'payment/default.htm'}else{virtPage='tilda/'+$jform.attr('id')+'submitted/default.htm'}}
